...
from models.AverageDepth import AverageDepth
...
args.data = "F:\Thesis\\4.Implementation\SfmLearner-Pytorch\datasets\Formatted_data"
args.width = 416
args.height = 128
args.dataset_format = "stacked"
args.sequence_length = 3
args.rotation_mode = 'euler'
args.paddig_mode = 'zeros'
args.with_gt = False
args.with_pose = False
args.load_save_data = True
args.workers = 4
args.epochs = 200
args.epoch_size = 0
args.batch_size = 4
...
args.pretrained_adabins = None
args.pretrained_resnet_encoder = None
args.pretrained_resnet_decoder = None
args.pretrained_avg_depth = "F:\Thesis\\4.Implementation\SfmLearner-Pytorch\pre_trained\Model_1\dispnet_model_best.pth.tar"
args.pretrained_exp_pose = "F:\Thesis\\4.Implementation\SfmLearner-Pytorch\pre_trained\Model_1\exp_pose_model_best.pth.tar"
args.seed = 0
args.log_summary = "progress_log_summary.csv"
args.log_full = "progress_log_full.csv"
args.photo_loss_weight =1
args.mask_loss_weight = 0
args.smooth_loss_weight = 0.1
args.sub_network_1_weight = 0.1
args.sub_network_2_weight = 0.7
args.sub_network_3_weight = 0.2
args.log_output = True
args.training_output_freq = 2000
args.save_path = "/content/drive/MyDrive/checkpoints/"
...
def main():
    ...
    print('{} validation samples found'.format(len(val_set)))
    ...
    avg_net = AverageDepth(args).to(device)
    ...
    if args.pretrained_avg_depth:
        print("=> using pre-trained weights for Dispnet")
        weights = torch.load(args.pretrained_disp)
        avg_net.load_state_dict(weights['state_dict'])
    else:
        avg_net.init_weights()

    cudnn.benchmark = True
    avg_net = torch.nn.DataParallel(avg_net)
    pose_exp_net = torch.nn.DataParallel(pose_exp_net)

    print('=> setting adam solver')

    optim_params = [
        {'params': avg_net.parameters(), 'lr': args.lr},
    ...
    if args.pretrained_disp or args.evaluate:
        ...
        if args.with_gt and args.with_pose:
            errors, error_names = validate_with_gt_pose(args, val_loader, avg_net, pose_exp_net, 0, logger, tb_writer)
        elif args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, avg_net, 0, logger, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, avg_net, pose_exp_net, 0, logger, tb_writer)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train(args, train_loader, avg_net, pose_exp_net, optimizer, args.epoch_size, logger, tb_writer)
        logger.train_writer.write(' * Avg Loss : {:.3f}'.format(train_loss))

        # evaluate on validation set
        logger.reset_valid_bar()
        if args.with_gt and args.with_pose:
            errors, error_names = validate_with_gt_pose(args, val_loader, avg_net, pose_exp_net, epoch, logger, tb_writer)
        elif args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, avg_net, epoch, logger, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, avg_net, pose_exp_net, epoch, logger, tb_writer)
        ...
        save_checkpoint(
            args.save_path, {
                'epoch': epoch + 1,
                'state_dict': avg_net.module.state_dict()
            }, {
                'epoch': epoch + 1,
...
def train(args, train_loader, avg_net, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
...
@torch.no_grad()
def validate_without_gt(args, val_loader, avg_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
...
@torch.no_grad()
def validate_with_gt_pose(args, val_loader, avg_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
...
@torch.no_grad()
def validate_with_gt(args, val_loader, avg_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
